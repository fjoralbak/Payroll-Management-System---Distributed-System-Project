...
public class StaffRepo {
    ...
    public static Staff findByName(String name) throws Exception {
        Connection conn = DbHelper.getConnection();
        PreparedStatement stmt = conn.prepareStatement("SELECT * FROM staff where emri =? ");
    ...
    private static String getProfessorsFromRes(ResultSet res) throws Exception {
        String name = res.getString("emri");
        return name;
    }

    public static ArrayList<String> findProfessors() throws Exception {
        ...
        while (res.next()) {
            professors.add(getProfessorsFromRes(res));
        }
        return professors;
    }

    private static Integer getProfessorIdFromRes(ResultSet res) throws Exception {
        Integer id = res.getInt("ssn");
        return id;
    }

    public static Integer getProfessorIdByProfessorName(String professorName) throws Exception {
        Connection conn = DbHelper.getConnection();
        PreparedStatement stmt = conn.prepareStatement("Select * from staff where emri = ?");
        ...
        return getProfessorIdFromRes(res);
    }
    ...
    /**
     public static ArrayList<String> findDepartments() throws Exception {
     ...
     while (res.next()) {
     departments.add(getDepartmentsFromRes(res));
     }
    ...
    /**
     public static Integer getDepartmentIdByDepartmentName(String departmentName) throws Exception {
     ArrayList<String> departments = new ArrayList<>();

     Connection conn = DbHelper.getConnection();
     PreparedStatement stmt = conn.prepareStatement("Select * from DEPARTMENT where departmentName = ?");
     stmt.setString(1, departmentName);

     ResultSet res = stmt.executeQuery();
     if (!res.next()) return null;

     return getDepartmentIdFromRes(res);
     }
     */
