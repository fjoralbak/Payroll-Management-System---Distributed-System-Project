...
public class ChatRepo {
    public static List<Chat> getCats(int senderId, int receiverId) throws Exception {
    ...
    public static List<Chat> getGroupChats() throws Exception {
        ArrayList<Chat> list = new ArrayList<>();

        Connection conn = DbHelper.getConnection();
        PreparedStatement stmt =
                conn.prepareStatement("SELECT * FROM groupChats "
                        + "ORDER BY sendTime");
        ResultSet res = stmt.executeQuery();
        while (res.next()) {
            list.add(parseGroupRes(res));
        }
        return list;
    }
    ...
    private static Chat parseGroupRes(ResultSet res) throws Exception {
        int senderId = res.getInt("senderId");
        String message = res.getString("message");
        Date sendTime = DateHelper.fromSql(res.getString("sendTime"));
        String path = res.getString("path");

        return new Chat(senderId, 0, sendTime, message, path);
    }
}
